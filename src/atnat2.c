...
/* round to nearest mode of IEEE 754 standard.                          */
/************************************************************************/
double atan2Mp(double ,double ,const int[]);
double signArctan2(double ,double);
double normalized(double ,double,double ,double);
void mpatan2(mp_no *,mp_no *,mp_no *,int);

double uatan2(double y,double x) {
...
double  normalized(double ax,double ay,double y, double z)
    { int p;
...
double signArctan2(double y,double z)
{
  return ((y<ZERO) ? -z : z);
}
  /* Stage 3: Perform a multi-Precision computation */
double  atan2Mp(double x,double y,const int pr[])
{
  double z1,z2;
  int i,p;
