...
void mpatan(mp_no *,mp_no *,int);          /* see definition in mpatan.c */
double atanMp(double,const int[]);
double signArctan(double,double);
/* An ultimate atan() routine. Given an IEEE double machine number x,    */
/* routine computes the correctly rounded (to nearest) value of atan(x). */
double uatan(double x) {
  ...
  if (u<C) {
    if (u<B) {
      ...
      if ((y=t1+(yy-u2*t1)) == t1+(yy+u2*t1))  return signArctan(x,y);
      ...
      if ((y=s2+(ss2-U6*s2)) == s2+(ss2+U6*s2))  return signArctan(x,y);
    ...
    if (u<D) { /* C <= u < D */     
      ...
      if ((y=t1+(yy-u3)) == t1+(yy+u3))  return signArctan(x,y);
      ...
      if ((y=s1+(ss1-U7)) == s1+(ss1+U7))  return signArctan(x,y);
    ...
    return atanMp(x,pr);   
      ...
      if (u<E) { /* D <= u < E */        
        ...
        ESUB(HPI,w,t3,cor)
        yy=((HPI1+cor)-ww)-yy;
        if ((y=t3+(yy-U4)) == t3+(yy+U4))  return signArctan(x,y);
        ...
        if ((y=s2+(ss2-U8)) == s2+(ss2+U8))  return signArctan(x,y);
...
double atanMp(double x,const int pr[]){
...
for (i=0; i<M; i++) {
    p = pr[i];
    dbl_mp(x,&mpx,p);          mpatan(&mpx,&mpy,p);
